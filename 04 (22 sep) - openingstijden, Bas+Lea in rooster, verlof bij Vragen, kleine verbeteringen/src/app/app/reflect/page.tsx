...
export default async function Reflect({ searchParams }: { searchParams: { saved?: string; w?: string } }) {
  ...
  return (
    <div className="max-w-3xl">
      ...
      {!rows.length && <Empty>{isAdmin ? tr("Noch keine Rückblicke.", "Nog geen weekboeken.") : tr("Noch kein Rückblick. Fang diese Woche an.", "Nog geen terugblik. Begin deze week.")}</Empty>}
      <div className="space-y-3">
        {rows.filter((r) => isAdmin || r.week !== week).map((r) => (
          <section key={r.id} className="card space-y-2">
            ...
            {isAdmin && <form action={saveReflectionFeedback} className="flex gap-2"><input type="hidden" name="id" value={r.id} /><input name="feedback" className="input" placeholder={tr("Antwort …", "Reactie (in het Duits) …")} defaultValue={r.feedback || ""} /><Submit className="btn-ghost">{tr("Senden", "Stuur")}</Submit></form>}
          </section>
        ))}
      </div>
    </div>
