    ...
    <div className="max-w-3xl">
      ...
      <section className="card mt-5">
        ...
        <form action={savePlan} className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto_auto]" key={plans.length}>
          ...
          {user.role === "admin" && <select name="user_id" className="input" defaultValue=""><option value="">Voor mezelf</option>{users.filter((u) => u.id !== user.id).map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select>}
