  ...
  return (
    <div>
      <PageHeader title={tr("Agenda", "Agenda")} sub={tr("Wer arbeitet wann, Termine, eingeplante Aufgaben und Storepflege – alles an einem Ort.", "Wie werkt wanneer, afspraken, ingeplande opdrachten en winkelverzorging – alles op één plek.")}>
        ...
        {isAdmin && <Link href={`/app/roster?w=${monday(sel)}`} className="btn-ghost btn-sm">Rooster vullen</Link>}
      </PageHeader>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
            ...
            {users.filter((u) => isAdmin || shiftOfSel(u.id)).map((u) => {
              ...
              return (
                <li key={u.id} className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                  ...
                  {isAdmin && <Link href={`${backUrl}&shift=${u.id}`} scroll={false} className="ml-auto text-xs text-brand hover:underline">Wijzig</Link>}
                </li>
              );
            })}
          ...
          {shiftUser && (
            <form action={saveShift} className="mt-3 space-y-2 rounded-xl bg-sand-100 p-3" key={`${shiftUser.id}${sel}`}>
              ...
              <select name="kind" className="input" defaultValue={shiftOfSel(shiftUser.id)?.kind || "shop"}>
                {Object.entries(SHIFT_KIND).map(([k, l]) => <option key={k} value={k}>{l.nl}</option>)}<option value="none">– Invoer verwijderen –</option>
              </select>
              <div className="grid grid-cols-2 gap-2">
                <div><label className="label">van</label><input type="time" name="start_time" className="input" defaultValue={hm(shiftOfSel(shiftUser.id)?.start_time) || "10:00"} /></div>
                <div><label className="label">tot</label><input type="time" name="end_time" className="input" defaultValue={hm(shiftOfSel(shiftUser.id)?.end_time) || (weekday(sel) === 6 ? "17:00" : "18:30")} /></div>
                <div><label className="label">pauze van</label><input type="time" name="break_start" className="input" defaultValue={hm(shiftOfSel(shiftUser.id)?.break_start)} /></div>
                <div><label className="label">pauze tot</label><input type="time" name="break_end" className="input" defaultValue={hm(shiftOfSel(shiftUser.id)?.break_end)} /></div>
              </div>
              <input name="note" className="input" placeholder="Notitie" defaultValue={shiftOfSel(shiftUser.id)?.note || ""} />
              <div className="flex gap-2"><Submit>Opslaan</Submit><Link href={backUrl} scroll={false} className="btn-ghost">Annuleren</Link></div>
            </form>
          )}
        ...
        <section className="card">
          ...
          <form action={savePlan} className="mt-3 space-y-2 rounded-xl bg-sand-100 p-3" key={`${sel}-${plans.length}`}>
            ...
            {isAdmin && <select name="user_id" className="input" defaultValue="">{<option value="">Voor mezelf</option>}{users.filter((u) => u.id !== user.id).map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select>}
