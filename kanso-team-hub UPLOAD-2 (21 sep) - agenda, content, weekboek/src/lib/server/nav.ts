...
export async function navItems(user: { id: string; role: string; lang: "de" | "nl" }): Promise<NavItem[]> {
  ...
  const G = { shop: tr("Laden", "Winkel"), proj: tr("Projekte", "Projecten"), me: user.role === "admin" ? "Stagiairs" : tr("Mein Praktikum", "Mijn stage"), admin: "Beheer" };
  const items: NavItem[] = [
  ...
  if (user.role === "admin") {
    items.push({ href: "/app/roster", label: "Rooster vullen", icon: "clock", group: G.admin });
    items.push({ href: "/app/team", label: "Team & instellingen", icon: "users", group: G.admin });
  }
  return items;
}
