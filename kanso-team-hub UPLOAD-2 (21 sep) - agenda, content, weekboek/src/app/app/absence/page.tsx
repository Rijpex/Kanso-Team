  ...
  return (
    <div className="max-w-3xl">
      ...
      <form action={requestAbsence} className="card mb-6 grid gap-3 sm:grid-cols-2" key={rows.length}>
        {isAdmin && <div className="sm:col-span-2"><label className="label">Voor wie</label><select name="user_id" className="input">{users.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></div>}
        ...
        <div><Submit>{isAdmin ? "Invoeren" : tr("Anfragen", "Aanvragen")}</Submit></div>
      </form>
      ...
      <ul className="space-y-2">
        {rows.map((r) => (
          <li key={r.id} className="flex flex-wrap items-center gap-2 rounded-xl bg-white p-3 text-sm ring-1 ring-sand-200">
            ...
            {isAdmin && r.status === "requested" && <form action={decideAbsence} className="flex gap-1"><input type="hidden" name="id" value={r.id} /><button name="status" value="approved" className="btn-primary btn-sm">Goedkeuren</button><button name="status" value="declined" className="btn-ghost btn-sm">Afwijzen</button></form>}
