          ...
          {shiftUser && (
            <form action={saveShift} className="mt-3 space-y-2 rounded-xl bg-sand-100 p-3" key={`${shiftUser.id}${sel}`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div><label className="label">{tr("von", "van")}</label><input type="time" name="start_time" className="input" defaultValue={hm(shiftOfSel(shiftUser.id)?.start_time) || "10:00"} /></div>
                <div><label className="label">{tr("bis", "tot")}</label><input type="time" name="end_time" className="input" defaultValue={hm(shiftOfSel(shiftUser.id)?.end_time) || (weekday(sel) === 6 ? "17:00" : "18:30")} /></div>
