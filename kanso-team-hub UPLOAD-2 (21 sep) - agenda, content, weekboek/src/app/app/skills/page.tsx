...
export default async function Skills() {
  ...
  return (
    <div className="max-w-4xl">
                ...
                {people.map((p) => {
                  ...
                  return (
                    <form key={p.id} action={setSkillLevel} className="mt-1.5 flex flex-wrap items-center gap-1.5">
                      ...
                      {isAdmin ? <span className="text-xs text-stone-400">zelf: {labels[cur?.level ?? 0]}</span> : (cur?.confirmed ?? 0) > 0 && <span className="badge bg-emerald-100 text-emerald-800">{tr("bestätigt", "bevestigd")}: {labels[cur!.confirmed]}</span>}
                    </form>
                  );
                })}
              </li>
      ...
      {isAdmin && (
        <details className="card"><summary className="cursor-pointer font-semibold">Leerdoel toevoegen</summary>
          <form action={saveSkill} className="mt-3 grid gap-2 sm:grid-cols-[12rem_1fr_auto]" key={skills.length}><input name="area" list="areas" className="input" placeholder="Gebied" required /><datalist id="areas">{areas.map((a) => <option key={a} value={a} />)}</datalist><input name="title_de" className="input" placeholder="Leerdoel (Duits)" required /><Submit>Toevoegen</Submit></form>
        </details>
      )}
    </div>
  );
